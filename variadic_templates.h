...
/**
 * @brief The Action class is a wrapper around a std::function<void(InputArgs...)> that offers additional utility, namely the && operator
 */
class Action {
...
public:
     ...
     */
    explicit Action(ActionFunction function)
        : m_function(function)
    {
    }

    operator ActionFunction()
    {
        return m_function;
    }
     ...
     */
    void operator()(const InputArgs&... args) const
    {
        m_function(std::forward<InputArgs>(args)...);
    }
    ...
    Action& operator+=(const Action& other)
    {
        ...
        this->m_function = [thisFunction, otherFunction](const InputArgs&... args) { thisFunction(args...); otherFunction(args...); };
        return *this;
    }
